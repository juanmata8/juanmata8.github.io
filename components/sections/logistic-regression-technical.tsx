...
const INSIGHTS: Record<string, string> = {
  A: "If a restaurant earned an A last time, it almost certainly will again. Past performance is the strongest signal we have — far stronger than what they cook or where they are.",
  B: "A previous B grade is a flashing red light. Once a restaurant slips below A, the model expects it to slip again — no matter the cuisine, no matter the neighborhood.",
  C: "A previous C is the loudest warning the data gives us. The kitchen has problems, and those problems tend to stick around.",
  First: "A brand-new restaurant has no track record yet. Cuisine and borough nudge the prediction a tiny bit — but without history, we're mostly guessing.",
}
      ...
      <div className="max-w-4xl mx-auto px-6">
        ...
        >
          ...
          <p className="font-serif text-lg text-muted-foreground leading-relaxed">
            We trained a simple model on 9,292 NYC restaurants to see what really predicts
            their next inspection grade. Pick a cuisine, a borough, and what the restaurant
            scored last time — then watch the prediction. You&apos;ll notice the first two
        ...
        >
          <p className="font-sans text-xs uppercase tracking-[0.4em] text-muted-foreground mb-2">
            How much each factor pulls
          </p>
          <p className="font-serif text-base text-muted-foreground leading-relaxed mb-10 max-w-2xl">
            Each bar shows how much that factor pushes the prediction up or down.
            We put all three groups on the same scale so you can compare at a glance.
            The story tells itself: history dwarfs everything else.
          </p>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-10">
            <CoefficientGroup
              title="Cuisine"
              subtitle="compared to American"
              data={CUISINE_COEFS}
              activeKey={selections.cuisine}
              scaleNote="Biggest pull: −1.46 (Indian)"
              isInView={isInView}
              delay={0.5}
            />
            <CoefficientGroup
              title="Borough"
              subtitle="compared to the Bronx"
              data={BORO_COEFS}
              activeKey={selections.borough}
              scaleNote="Biggest pull: −0.29 (Queens)"
              isInView={isInView}
              delay={0.6}
            />
            <CoefficientGroup
              title="Past grade"
              subtitle="compared to first inspection"
              data={HISTORY_COEFS}
              activeKey={selections.history}
              scaleNote="Biggest pull: −8.72 — six times bigger than any cuisine"
              isInView={isInView}
              delay={0.7}
            />
          </div>
